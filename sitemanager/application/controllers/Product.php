...
//
	...
	public function index() {
		...
		$data['categories']           = $this->categoryModel->allCategory_array_name();
	...
	public function excelUploadOnly() {
            ...
            if ($this->upload->do_upload('file')) {
                ...
                // $output['status'] = true;
               $output['status']=  $this->uploadExcel($output['file']);
	...
	public function uploadExcel($excel) {
		...
		if ($this->input->post()) {
			...
			if ($category->unique_id=='mob-ac' || $category->unique_id=='mob-acc') {
				...
				$output['status'] = $this->upload_mobile_acc($excel);
			}

			if ($category->unique_id=='ca-co') {
				
				$output['status'] = $this->upload_cases_cover($excel);
			}

			if ($category->unique_id=='po-ba') {
				
				$output['status'] = $this->upload_power_bank($excel);
			}

			if ($category->unique_id=='cable') {
				
				$output['status'] = $this->upload_cable($excel);
			}
			if ($category->unique_id=='se-st') {
				
				$output['status'] = $this->upload_selfie_stick($excel);
			}
			if ($category->unique_id=='mob-char') {
				
				$output['status'] = $this->upload_mobile_charger($excel);
			}

			if ($category->unique_id=='mob-len') {
				
				$output['status'] = $this->upload_mobile_lense($excel);
			}
			if ($category->unique_id=='battery') {
				
				$output['status'] = $this->upload_battery($excel);
			}
			if ($category->unique_id=='sc-gu') {
				
				$output['status'] = $this->upload_screen_guard($excel);
			}
			if ($category->unique_id=='mo-sk') {
				
				$output['status'] = $this->upload_mobile_skin($excel);
			}
			if ($category->unique_id=='ca-re') {
				
				$output['status'] = $this->upload_card_reader($excel);
			}
			if ($category->unique_id=='sp-pa') {
				
				$output['status'] = $this->upload_spare_parts($excel);
			}

			if ($category->unique_id=='mo-ho') {
				
				$output['status'] = $this->upload_mobile_holder($excel);
			}

			if ($category->unique_id =='lens-cleaner' || $category->unique_id=='lens-hood') {

				$output['status'] = $this->upload_lens_cleaner($excel);
			}
